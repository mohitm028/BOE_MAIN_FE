...
export interface NavSectionProps {
  ...
  expanded?: boolean
  onClick?: () => void
  onSubItemClick?: (label: string, path?: string, item?: SubItem) => void
}
    ...
    <div className="px-4 py-2">
      ...
      >
        <div className="w-6 text-center">
          {typeof icon === 'string' ? icon : <IconComponent size={16} />}
        </div>
        ...
        <ul className="pl-8 mt-2 space-y-1">
          {subItems.map((item, index) => (
            <li
              ...
              onClick={() => onSubItemClick?.(item.label, item.path, item)}
            >
              {item.label}
            </li>
          ))}
